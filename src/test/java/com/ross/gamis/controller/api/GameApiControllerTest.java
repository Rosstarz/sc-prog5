...
class GameApiControllerTest {
    ...
    public void getGameByIdShouldReturnGameForExistentGame() throws Exception {
        mockMvc.perform(
                ...
                .andExpect(jsonPath("$.stores[*].store.name", Matchers.containsInRelativeOrder(
                        createdStoreOne.getName(), createdStoreTwo.getName(), createdStoreThree.getName())));
    }
    ...
    public void getGameByIdAndAcceptXmlShouldReturnGameInXml() throws Exception {
        mockMvc.perform(
                ...
                .andExpect(status().isOk())
                .andExpect(content().contentType(new MediaType(MediaType.APPLICATION_XML.getType(),
    ...
    public void getAllGamesShouldReturnAllGames() throws Exception {
        mockMvc.perform(
                ...
                .andExpectAll(
                        ...
                        jsonPath("$[*].developer.developerId").isNotEmpty(),
                        jsonPath("$[*].developer.name").isNotEmpty(),
                        jsonPath("$[*].stores").isNotEmpty());
                // .andReturn();

        // MatcherAssert.assertThat(games, hasItems(
        //         Matchers.samePropertyValuesAs(
        //                 new GameDtoOut(createdGame.getId(), createdGame.getTitle(), createdGame.getDescription()), "developer",
        //                 "stores")));
    }
    ...
    public void addGameShouldBeCreatedIfCorrectBody() throws Exception {
        var mvcResult = mockMvc.perform(
                post("/api/games")
                        ...
                        .accept(MediaType.APPLICATION_JSON)
                        .content(
                                "{\"title\": \"New Game\", \"description\": \"New Game Description\", \"developerId\": "
